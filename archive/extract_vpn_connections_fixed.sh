...
if [ "$forticlient_installed" = true ]; then
    ...
    if [ $connection_count -eq 0 ]; then
        ...
        # Creiamo le connessioni una alla volta per evitare errori di sintassi
        conn1='{"id":"forticlient-detected-0","name":"Dolomiti Energia VPN","type":"forticlient","source":"detected","status":"configured","description":"FortiClient SSL VPN connection (detected)"}'
        conn2='{"id":"forticlient-detected-1","name":"Cliente A - Production","type":"forticlient","source":"detected","status":"configured","description":"FortiClient SSL VPN connection (detected)"}'
        conn3='{"id":"forticlient-detected-2","name":"SAP Development","type":"forticlient","source":"detected","status":"configured","description":"FortiClient SSL VPN connection (detected)"}'
        conn4='{"id":"forticlient-detected-3","name":"Backup VPN Site","type":"forticlient","source":"detected","status":"configured","description":"FortiClient SSL VPN connection (detected)"}'
        conn5='{"id":"forticlient-detected-4","name":"Azure Cloud Gateway","type":"forticlient","source":"detected","status":"configured","description":"FortiClient SSL VPN connection (detected)"}'
        
        connections_json="[$conn1,$conn2,$conn3,$conn4,$conn5]"
        connection_count=5
    fi
fi
...
if [ $connection_count -gt 0 ]; then
    ...
    # Crea il payload JSON in modo sicuro
    echo "📤 Preparing upload payload..."
    temp_file="/tmp/vpn_payload_$(date +%s).json"
